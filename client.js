...
(function () {
  ...
  // ---------- 全螢幕 ----------
  // 瀏覽器規定一定要玩家點一下才能進全螢幕，所以轉橫向時先「預備」，下一次點畫面就進全螢幕
  const docEl = document.documentElement;
  const fsOk = !!(docEl.requestFullscreen || docEl.webkitRequestFullscreen);
  const isFs = () => !!(document.fullscreenElement || document.webkitFullscreenElement);
  const isTouch = () => (navigator.maxTouchPoints || 0) > 0 || 'ontouchstart' in window;
  const isLand = () => window.matchMedia('(orientation: landscape)').matches;
  // 從主畫面圖示打開（本來就是全螢幕）；網頁進全螢幕時瀏覽器也會回報 fullscreen，所以要排除
  const isApp = () => !!navigator.standalone || (!isFs() && window.matchMedia('(display-mode: fullscreen), (display-mode: standalone)').matches);
  const isIOS = () => /iPhone|iPod/.test(navigator.userAgent) || (/Macintosh/.test(navigator.userAgent) && isTouch() && !fsOk);
  let fsArmed = false, fsAuto = false;
  function enterFs() {
    if (!fsOk || isFs()) return;
    try {
      const r = (docEl.requestFullscreen ? docEl.requestFullscreen({ navigationUI: 'hide' }) : docEl.webkitRequestFullscreen());
      if (r && r.catch) r.catch(() => { });
    } catch (e) { }
  }
  function exitFs() { try { if (isFs()) { const r = (document.exitFullscreen || document.webkitExitFullscreen).call(document); if (r && r.catch) r.catch(() => { }); } } catch (e) { } }
  function toggleFs() {
    $('menu').classList.add('hidden');
    if (!fsOk) {
      if (isApp()) return;
      dialog(isIOS()
        ? 'iPhone 的瀏覽器不支援網頁全螢幕。<br>請用 Safari 打開遊戲 → 按下方的「分享」→「加入主畫面」，之後從主畫面的麻將圖示打開，就是全螢幕。'
        : '這個瀏覽器不支援全螢幕。可以把遊戲「加入主畫面」，從主畫面打開就是全螢幕。', [['知道了', null]]);
      return;
    }
    fsAuto = false;
  ...
  function fsLabel() {
    ...
    $('lobbyFs').textContent = on ? '離開全螢幕' : '全螢幕';
    $('lobbyFs').classList.toggle('hidden', !!isApp());
    $('fsTop').classList.toggle('hidden', !!isApp() || isFs());
    if (isFs()) { fsArmed = false; $('fsHint').classList.add('hidden'); }
  }
  function onOrient() {
    if (!isTouch() || !fsOk || isApp()) return;
    if (isLand()) {
      if (!isFs()) { fsArmed = true; fsAuto = true; enterFs(); setTimeout(() => { if (fsArmed && !isFs()) { $('fsHint').classList.remove('hidden'); setTimeout(() => $('fsHint').classList.add('hidden'), 3500); } }, 300); }
    } else {
      fsArmed = false; $('fsHint').classList.add('hidden');
      if (fsAuto && isFs()) exitFs();
      fsAuto = false;
    }
  }
  // 預備好之後，玩家第一次點畫面就進全螢幕（同一下點擊的原本功能照常執行）
  const armTap = () => { if (fsArmed && isLand() && !isFs()) { fsArmed = false; $('fsHint').classList.add('hidden'); enterFs(); } };
  document.addEventListener('pointerup', armTap, true);
  document.addEventListener('touchend', armTap, true);
  ...
  $('lobbyFs').onclick = (e) => { e.stopPropagation(); toggleFs(); };
  $('fsTop').onclick = (e) => { e.stopPropagation(); toggleFs(); };
  fsLabel();
  if (isLand()) { if (isTouch() && fsOk && !isApp()) { fsArmed = true; fsAuto = true; } }
  function quit() {
    $('menu').classList.add('hidden');
    $('result').classList.add('hidden');
