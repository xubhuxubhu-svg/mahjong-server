...
function wireRoomEvents(io, room) {
  ...
  room.on("meldFormed", (payload) => broadcastPublic(io, room, "meld_formed", payload));
}

function startServer(port) {
  const httpServer = http.createServer((req, res) => {
    // 直接用瀏覽器打開這個網址時，顯示一句確認訊息（真正的連線是靠socket.io，不是靠這個網頁）
    res.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" });
    res.end("🀄 台灣16張麻將伺服器運作中。請用遊戲畫面（mahjong_client.html）連線，不要直接用瀏覽器開這個網址。");
  });
  ...
  io.on("connection", (socket) => {
    socket.on("create_room", ({ name } = {}, cb) => {
      ...
      const room = new Room(roomId, { aiDelayMs: 900 });
